<?php
// ==============================================================================
# Action Handler: Reboot ONU
// ==============================================================================
set_time_limit(0); // SSH reboot command can exceed 30s default
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        ...
        if ($onu) {
            ...
            // Kirim perintah reboot ke OLT
            $res = reboot_onu($olt, $onu);
            ...
            if ($res['success']) {
                ...
                // Audit Log
                write_audit_log($onu['olt_id_raw'], 'ONU_REBOOT', "ONT {$onu['serial_number']} ({$onu['name']}) di-reboot secara manual oleh administrator.");

                $_SESSION['success'] = "ONT berhasil dikirimi perintah reboot! Koneksi ONT sedang merestart.";
            } else {
                $_SESSION['error'] = "Gagal me-reboot ONT: " . $res['message'];
            }
